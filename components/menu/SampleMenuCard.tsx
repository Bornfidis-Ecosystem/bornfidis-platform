...
export function SampleMenuCard({ title, lines, note }: SampleMenuCardProps) {
  return (
    <BrandedCard theme="culinary" className="flex h-full flex-col">
      <h2 className="font-display text-xl font-semibold text-[#1a1a1a] md:text-2xl">{title}</h2>
      <ul className={`${bookBody} mt-4 flex-1 space-y-2 text-sm`}>
        {lines.map((line) => (
          <li key={line} className="flex gap-2">
            <span className="text-[#ffbc00]">—</span>
            <span>{line}</span>
          </li>
        ))}
      </ul>
      <p className="mt-6 border-t border-[#ffbc00]/35 pt-4 font-sans text-xs text-[#1a1a1a]/55">{note}</p>
    </BrandedCard>
  )
}
