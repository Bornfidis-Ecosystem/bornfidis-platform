        ...
        <PageContainer wide>
          ...
          {!rich ? (
            <article className={`${academyCard} mt-6 max-w-3xl`}>
              <div className="relative aspect-video bg-[#2c2c2c]/5">
                {product.image ? (
                  <Image src={product.image} alt="" fill className="object-cover" sizes="896px" priority />
                ) : (
                  <div className="flex h-full items-center justify-center font-display text-6xl text-[#2c2c2c]/25">
                    {product.title.charAt(0)}
                  </div>
                )}
                <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                  <span className="bg-[#1A3C34] px-3 py-1.5 font-sans text-[10px] font-semibold uppercase tracking-[0.1em] text-[#fdf8f8]">
                    {product.category}
                  </span>
                  {product.type !== 'DOWNLOAD' ? (
                    <span className="border border-[#C9A84C]/50 bg-[#fdf8f8] px-3 py-1.5 font-sans text-[10px] font-semibold uppercase tracking-[0.1em] text-[#2c2c2c]">
                      {product.type === 'COURSE' ? 'Course' : 'Bundle'}
                    </span>
                  ) : null}
                </div>
              </div>
              <div className="border-t border-[#C9A84C]/25 p-8">
                <h1 className={`${academyHeadline} text-2xl`}>{product.title}</h1>
                <p className={`${academyBody} mt-4`}>{product.description}</p>
                <div className="mt-6 flex flex-wrap items-center gap-4">
                  <span className="font-display text-xl text-[#2c2c2c]">{product.priceDisplay}</span>
            ...
            <>
              <section className="mt-8 grid grid-cols-1 items-start gap-10 md:grid-cols-5 md:gap-12">
                <div className="relative aspect-[3/4] max-w-sm overflow-hidden border border-[#C9A84C]/35 md:col-span-2">
                  {product.image ? (
                    <Image
                      src={product.image}
                      alt={product.title}
                    ...
                    <div className="flex h-full items-center justify-center font-display text-5xl text-[#2c2c2c]/25">
                      {product.title.charAt(0)}
                    </div>
                  )}
                </div>
                <div className="md:col-span-3">
                  ...
                  <p className="mt-6 font-display text-2xl text-[#2c2c2c]">{product.priceDisplay}</p>
                  <div className="mt-6">
                    <AcademyBuyButton product={product} size="lg" />
                  </div>
                  <p className={`${academyBody} mt-4 text-sm`}>
              ...
              <section className="mt-16 max-w-3xl">
                ...
                {product.targetAudience && product.targetAudience.length > 0 ? (
                  <div className="mt-10">
                    <h3 className="font-display text-lg text-[#2c2c2c]">Who this is for</h3>
                    <ul className="mt-3 flex flex-wrap gap-2">
                      {product.targetAudience.map((aud) => (
                        <li
                          key={aud}
                          className="border border-[#C9A84C]/35 px-3 py-1.5 font-sans text-sm text-[#2c2c2c]"
                        >
                          {aud}
                        </li>
                      ))}
                ...
                {product.learningOutcomes && product.learningOutcomes.length > 0 ? (
                  <div className="mt-10">
                    <h3 className="font-display text-lg text-[#2c2c2c]">What you&apos;ll learn</h3>
                ...
                {product.whatIsIncluded && product.whatIsIncluded.length > 0 ? (
                  <div className="mt-10">
                    <h3 className="font-display text-lg text-[#2c2c2c]">What&apos;s included</h3>
                    <ul className={`${academyBody} mt-3 list-disc space-y-2 pl-5 text-sm`}>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {TESTIMONIAL_PLACEHOLDERS.slice(0, 5).map((t, i) => (
                    <blockquote key={i} className={`${academyCard} p-6`}>
                      ...
                      <footer className="mt-4 font-sans text-sm font-semibold text-[#2c2c2c]">
                        {t.name} — {t.role}
                      </footer>
                    </blockquote>
                  ))}
              ...
              <section className="mt-16 max-w-3xl">
                ...
                <dl className="mt-6 space-y-6">
                  {FAQ_ITEMS.map((faq) => (
                    <div key={faq.q} className="border-b border-[#C9A84C]/25 pb-6 last:border-0">
                      <dt className="font-display text-lg text-[#2c2c2c]">{faq.q}</dt>
              ...
              <section className="mt-16 border border-[#C9A84C]/35 p-8 text-center md:p-12">
                <h2 className={academyHeadline}>Ready to build with structure?</h2>
                <p className="mt-4 font-display text-3xl text-[#2c2c2c]">{product.priceDisplay}</p>
