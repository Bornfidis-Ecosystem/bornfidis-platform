  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-8">
      ...
      <p className="text-gray-600 text-center max-w-md">
        We couldn’t load the admin area. This is often due to a missing or invalid session or a temporary server issue.
      </p>
      <Link href="/admin/login" className="text-green-800 font-medium underline mt-2">
        Go to login
      </Link>
    </div>
  )
...
export default async function AdminLayout({
  ...
  try {
    let result: Awaited<ReturnType<typeof checkAdminAccess>>
    try {
