...
import { createDefaultPrepTasks } from '@/lib/prep-tasks'
...
/**
 * Provisions Stripe webhook — private dining deposits, balances, Connect.
 * POST /api/stripe/provisions/webhook
 *
...
export async function POST(request: NextRequest) {
    ...
    try {
        stripe = getStripeClient('provisions')
        webhookSecret = getWebhookSecret('provisions')
    } catch (err) {
        console.error('[provisions webhook]', err instanceof Error ? err.message : err)
        return NextResponse.json({ error: 'Stripe not configured for provisions' }, { status: 500 })
    }
    ...
    try {
        ...
        console.error('Provisions webhook signature verification failed:', err.message)
        ...
        try {
            if (isDeposit) {
                const { data: currentBooking, error: depositFetchError } = await supabaseAdmin
                    ...
                    .select('id, paid_at, email, name, event_date')
                    .eq('id', bookingId)
                    .single()
                ...
                if (!previousDepositPaid) {
                    ...
                    try {
                        await createDefaultPrepTasks(bookingId, {
                            eventDate: currentBooking.event_date
                                ? new Date(currentBooking.event_date)
                                : undefined,
                            actorName: 'Stripe webhook',
                        })
                    } catch (prepError) {
                        console.error(
                            'Error creating default prep tasks for booking',
                            bookingId,
                            prepError,
                        )
                        // Do not fail the webhook — payment is already recorded.
                        // Prep tasks can be backfilled later; this is non-fatal.
                    }
