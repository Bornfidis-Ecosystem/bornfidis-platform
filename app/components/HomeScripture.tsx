export default function HomeScripture() {
  return (
    <section className="py-16 md:py-20 bg-cream/20 border-y-2 border-gold">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-2xl md:text-3xl font-semibold text-forestDark mb-4 italic">
            "Go therefore and make disciples of all nations, baptizing them in the name of the Father
            and of the Son and of the Holy Spirit, teaching them to observe all that I have commanded you."
          </p>
          <p className="text-lg text-gray-700 font-semibold">— Matthew 28:19-20</p>
          <div className="h-1 w-24 bg-forestDark mx-auto mt-6"></div>
        </div>
      </div>
    </section>
  )
}

